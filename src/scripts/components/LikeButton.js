...
class LikeButton extends HTMLElement {
  ...
  async _initLike () {
    const exist = await favoritesService.detail(this._restaurant.id);
    if (exist) {
      this._liked = true;
      this.render();
    }
  ...
  _afterRender () {
    this.querySelector('button').addEventListener('click', async () => {
      if (this._restaurant) {
        if (this._liked) {
          await favoritesService.delete(this._restaurant.id);
        } else {
  ...
  render () {
    const icon = this._liked
      ...
      `
      : `
        <svg xmlns="http://www.w3.org/2000/svg" height="1em" fill="none" viewBox="0 0 24 24" stroke="currentColor">
